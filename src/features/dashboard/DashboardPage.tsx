  ...
  useEffect(() => {
    const cargarDatosDashboard = async () => {
      ...
      try {
        ...
        setErrorConexion(
          error?.message ||
            "Ocurrió un retardo inesperado al jalar los datos desde la boveda de Supabase.",
        );
      } finally {
        setLoading(false);
      }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {errorConexion && (
          <button
            ...
            className="bg-primary text-surface text-xs font-semibold px-4 py-2 rounded-lg hover:bg-primary/90 transition-all shadow-xs shrink-0"
          >
      ...
      {errorConexion && (
        <div className="animate-in fade-in duration-200 max-w-4xl">
          <AlertNotification
            ...
            message={`${errorConexion} Algunos módulos visuales podrían mostrar métricas parciales.`}
            onClose={() => setErrorConexion(null)}
          />
        </div>
      )}
