...
import { useDetalleInformeImpuesto } from "./useInformes";
...
export const DetalleInformeImpuestoPage = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = useDetalleInformeImpuesto(id, fechaInicio, fechaFin);

  if (isLoading) {
    return (
      <Loader
        texto="Cargando auditoría detallada de la obligación tributaria..."
        fullScreen={false}
      />
    );
  }
    ...
    return (
      <div className="bg-surface p-8 rounded-xl border border-danger/30 text-center space-y-4">
        ...
        <p className="text-text-muted text-sm">
          El registro del impuesto no existe o hubo un error en la conexión.
        </p>
        <button
          onClick={() => navigate(-1)}
  ...
  const { impuesto, metricas, items } = resumen;
  ...
  const itemsFiltrados = items.filter((item) => {
    ...
    const coincideTexto =
      ...
      item.contador_nombre.toLowerCase().includes(busquedaTexto.toLowerCase());
    return coincideClase && coincideTexto;
  });

  const handleExportar = () => {
    exportService.exportarDetalleImpuestoExcel(
      itemsFiltrados,
      impuesto.nombre,
      fechaInicio,
      fechaFin,
      ...
      <div className="bg-surface border border-text-muted/20 rounded-xl shadow-xs overflow-hidden space-y-4 p-4">
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 border-b border-text-muted/10 pb-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder="Buscar por cliente, NIT o contador..."
              value={busquedaTexto}
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="bg-background/50 text-text-muted text-xs uppercase tracking-wider border-b border-text-muted/10">
                ...
                <th className="px-4 py-3 font-semibold">Contador Asignado</th>
